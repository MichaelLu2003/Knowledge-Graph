...
def generate_object(entity_label, relation_label):
    ...
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "Answer the query in the strict format: A: 'object'"},
            {"role": "user", "content": prompt}
        ],
        max_tokens=100,
        stop=["\nQ:", "\nA:"] 
    ...
    while not queue.empty():
        ...
        for relation_label in common_relation_labels:
            ...
            for relation_paraphrase in relation_paraphrases:
                generated_object = generate_object(current_label, relation_paraphrase)
...
def main():
    ...
    max_depth = 2
    graph = construct_knowledge_graph(root_entity_label, max_depth, branch_limit = 1)
